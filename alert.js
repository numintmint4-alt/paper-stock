...
function addAlertReceiveDate() {
  ...
  renderAlertDateTabs();   // ✅ render Tab (รวมปุ่ม +)
}
...
function renderAlertReceiveDates() {
  // ✅ ไม่ต้อง render ที่นี่แล้ว — ย้ายไปรวมใน renderAlertDateTabs()
  renderAlertDateTabs();
}
...
function renderAlertDateTabs() {
  ...
  // ✅ ปุ่ม "+ เพิ่มวันที่รับสินค้า" ต่อท้าย Tab
...
async function renderAlert() {
  ...
  try {
    ...
    // ✅ อัปเดต Size List จากผลลัพธ์ RPC
    ...
    // ✅ ลบ alertReceiveDateBox ออกจาก alert-summary
    ...
    // ✅ หัวคอลัมน์ Gradegrams — มี Filter
    ...
    // ✅ Render filter lists + attach dropdown ในหัวคอลัมน์
    renderAlertGradeList();
    renderAlertFilterList();
    renderAlertSizeList();
    updateAlertGradeLabel();
