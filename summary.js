// ═══════════════════════════════════════════════════════════════
//  STOCK ม้วนกระดาษ V6 — summary.js
//  ส่วนที่ 7: หน้า Receive Summary (ตารางสรุปรายการสั่งเข้า)
// ═══════════════════════════════════════════════════════════════
...
function updateSummaryGradeLabel() {
  ...
  if (summarySelectedGrades.length === 0) {
    ...
    label.textContent = summarySelectedGrades[0];
    label.style.color = '#1e40af';
  } else {
    label.textContent = `เลือก ${summarySelectedGrades.length} เกรด`;
    label.style.color = '#1e40af';
  }
}
...
async function renderSummary() {
  ...
  const modeLabel = mode === 'qty' ? 'จำนวน' :
                    mode === 'kg' ? 'KG รวม' : 'จำนวน + KG';
  ...
  try {
    // ✅ ใช้ get_receive_list ที่มีอยู่แล้ว — กรองวันที่เดียว
    const { data, error } = await supabase.rpc('get_receive_list', {
      p_date_from: summaryDate,
      p_date_to: summaryDate,
      p_grades: summarySelectedGrades.length > 0 ? summarySelectedGrades : null,
      p_suppliers: null,
      p_fsc: fsc,
